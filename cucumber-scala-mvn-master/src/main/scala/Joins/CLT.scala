...
import Functions.{Covref,TargetAcct,AccountLvlTarget,ModellingVersion,CustPCT,Cust,BMDivPCT,TargetAccountCMRMapping,ConfigCycle,CountryRollUpGeo,ParticipatingMarket,AttributeValue,ReadTable}
...
class CLT {
  ...
  def joinTargetAccountWithValidCoveref(EFFC_DATE_SQL: Date, REQ_IMT_ID: Int): DataFrame = {     
    ...
    val DF_VALID_COVEREF = covref.getValidCoveref(EFFC_DATE_SQL)
  ...
  def joinValidAccountLevelTargetwithModelingVersion(EFFC_DATE_SQL: Date, REQ_IMT_ID: Int): DataFrame = {
   ...
   def joinTargetAccountWithCUSTPCT(EFFC_DATE_SQL: Date, REQ_IMT_ID: Int): DataFrame ={
    ...
    val DF_TARGET_ACCOUNT = joinValidAccountLevelTargetwithModelingVersion(EFFC_DATE_SQL,REQ_IMT_ID)
    val DF_CUST_PCT = custPCT.getCustPCT(EFFC_DATE_SQL,REQ_IMT_ID)

    DF_TARGET_ACCOUNT.join(DF_CUST_PCT,Seq("TARGET_ACCOUNT_ID","BRAND_SUB_SUBGROUP_ID","ITT_ATTRIB_VALUE_ID_TGT","IMT_ID"))
  }
   ...
   var df= DF_VALID_CUST.join(DF_VALID_ITT_CUST,Seq("ITT_CUST_ID"))
    ...
    df =  df.withColumn("ZERO_REV_FACTOR",when(df("CNT_CUSTOMER")<=>0 , lit(1.0)/df("CNT_CUSTOMER")))
...
def getCustPCTNoRev(): DataFrame = {
  ...
  val var1 = vld_trgt.select("TARGET_ACCOUNT_ID", "BRAND_SUB_SUBGROUP_ID", "ITT_ATTRIB_VALUE_ID_TGT", "IMT_ID")

  val No_PCT = var1.except(vld_cust)
  ...
  No_PCT
    ...
    def getRegisteredSubmarket(): DataFrame = {
